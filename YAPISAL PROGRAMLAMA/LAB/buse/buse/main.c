/*
SORU:
Verilen 3 boyutlu noktalar˝n orjine olan uzakl˝klar˝n˝ Selection Sort algoritmas˝yla b¸y¸kten k¸Á¸e
veya k¸Á¸kten b¸y¸e s˝ralayan bir algoritmay˝ C dilinde gerÁekleyelim.
*/
...
int main(){
    ...
    scanf("%d", &order);
    //int (*f[2])(float, float) = {ascending, descending};
    //selectionSort(a, arraySize, f[order-1]);
    if(order == 1)
    {
...
void selectionSort(struct point *array, const int size, int (*compare)(float distance1, float distance2))
{
    ...
    for(i=0; i < size-1; i++)
    {
        ...
        for(j=i+1; j < size; j++)
        {
            ...
            if(!compare(d1, d2)) //if(d1>d2) //if(!f[0])
                smallestOrLargest = j;
        }
